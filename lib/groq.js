...
Keep lessons short, engaging, and easy to read.`;
  ...
  if (!response.ok) {
    ...
    // Specifically catch and enhance the invalid API key error to be more helpful
    if (errorMessage.toLowerCase().includes('invalid api key')) {
      errorMessage = 'Invalid API Key. Groq likely revoked your key for security reasons (e.g., exposed on GitHub or pasted publicly). Get a new one at https://console.groq.com/keys and update your Vercel Environment Variables and local .env file.';
    }
